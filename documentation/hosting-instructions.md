...
- ssh into the server
    - For Mac and Linux: `ssh root@beadsbybey.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo aanwar`
    - replace aanwar with your own
- create password
    - `passwd aanwar`
- Move .ssh key file
    - `cp -r .ssh /home/aanwar`
- change ownership of the .ssh directory
    - `cd /home/aanwar`
    - `chown -R aanwar:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh aanwar@beadsbybey.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- `sudo vim /etc/nginx/sites-available/beadsbybey.com`
...
server {
    ...
    root /var/www/beadsbybey.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name beadsbybey.com www.beadsbybey.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/beadsbybey.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/aanwar/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/aanwar/.ssh/github_deploy_key.pub >> /home/aanwar/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/aanwar/.ssh/github_deploy_key`
